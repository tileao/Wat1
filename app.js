...
const offshoreChartImage = new Image();
offshoreChartImage.src = OFFSHORE_STANDARD_EXACT.image;
...
function drawBackgroundImage() {
  if (!offshoreChartImage.complete) return false;
  ctx.drawImage(offshoreChartImage, 0, 0, chartCanvas.width, chartCanvas.height);
  return true;
}

function mapPdfToCanvasX(x) {
  const clip = { x0: 35, y0: 145, x1: 320, y1: 505 };
  return ((x - clip.x0) / (clip.x1 - clip.x0)) * chartCanvas.width;
}
function mapPdfToCanvasY(y) {
  const clip = { x0: 35, y0: 145, x1: 320, y1: 505 };
  return ((y - clip.y0) / (clip.y1 - clip.y0)) * chartCanvas.height;
}
...
function drawOverlay(result) {
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  roundRect(ctx, 18, 18, chartCanvas.width - 36, 96, 16);
  ctx.fill();
  ctx.stroke();
...
function runCalculation() {
  ...
  if (!(procedure === 'offshore' && configuration === 'standard')) {
    statusCard.className = 'card status neutral';
    statusBadge.textContent = 'STANDARD EXATO APENAS';
    statusTitle.textContent = 'Modo ainda não calibrado';
    statusText.textContent = 'Nesta etapa, somente Offshore Helideck + Standard usa extração vetorial direta do PDF.';
    maxWeightEl.textContent = '—';
    marginEl.textContent = '—';
    drawOverlay();
    return;
...
offshoreChartImage.addEventListener('load', () => {
  if (!chartPanel.classList.contains('hidden')) drawOverlay();
});
