...
const PAGE_CHART_PLACEMENT = { scale: 0.740971986, offsetX: 273.206409, offsetY: 594.946143, width: 285 * 0.740971986, height: 360 * 0.740971986 };
const CHART_CLIP_BOUNDS = { x0: 35, y0: 145, x1: 320, y1: 505 };
...
function syncProfileAssets() {
  const profile = getActiveProfile();
  const pageSrc = profile?.page || 'docs/page-07.png';
  chartBaseImage.src = pageSrc;
  chartBaseImage.alt = profile ? `Página do manual ${getCurrentProfileTitle()}` : 'Página do gráfico';
  chartExportPageImage.src = pageSrc;
  if (chartReferenceEl) {
...
function paToY(paFt, profile = getActiveProfile()) {
  const m = profile.main;
  const minPaFt = m.minPaFt ?? 0;
  const maxPaFt = m.maxPaFt;
  const yMinFt = m.yMinFt ?? profile.headwind.yTop;
  const pa = clamp(paFt, minPaFt, maxPaFt);
  const t = (pa - minPaFt) / (maxPaFt - minPaFt);
  return yMinFt + (1 - t) * (m.yTopFt - yMinFt);
}
...
function getHeadwindAdjustedLimit(baseKg, headwindKt, profile = getActiveProfile()) {
  ...
  if (hw <= 0) {
    return { hwY: profile.headwind.yTop, familyBases: [], familyValues: [], maxKg: baseKg };
  }
...
function pdfPointToPage(x, y) {
  return {
    x: PAGE_CHART_PLACEMENT.offsetX + ((x - CHART_CLIP_BOUNDS.x0) / (CHART_CLIP_BOUNDS.x1 - CHART_CLIP_BOUNDS.x0)) * PAGE_CHART_PLACEMENT.width,
    y: PAGE_CHART_PLACEMENT.offsetY + ((y - CHART_CLIP_BOUNDS.y0) / (CHART_CLIP_BOUNDS.y1 - CHART_CLIP_BOUNDS.y0)) * PAGE_CHART_PLACEMENT.height
  };
}

function mapPdfToCanvasX(x) {
  const p = pdfPointToPage(x, CHART_CLIP_BOUNDS.y0);
  return (p.x / chartBaseImage.naturalWidth) * chartCanvas.getBoundingClientRect().width;
}
function mapPdfToCanvasY(y) {
  const p = pdfPointToPage(CHART_CLIP_BOUNDS.x0, y);
  return (p.y / chartBaseImage.naturalHeight) * chartCanvas.getBoundingClientRect().height;
}
...
function drawOverlay(result) {
  ...
  if (!result || result.error) {
    ...
    ctx.fillText('Visualização do gráfico do RFM', 32, 48);
    ctx.fillStyle = '#8ea0b7';
    ctx.font = '20px Inter, system-ui, sans-serif';
    ctx.fillText('Rodando o cálculo, a interpolação aparece sobre a página inteira do manual.', 32, 84);
    return;
  }
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  roundRect(ctx, 18, 18, rect.width - 36, 110, 16);
  ctx.fill();
  ctx.stroke();
  ...
  ctx.fillText(`No wind ${Math.round(result.noWind.noWindKg)} kg -> Final ${Math.round(result.maxWeight)} kg`, 32, 104);
  ctx.restore();
}
...
function renderCompositeCanvas(result = currentResult) {
  ...
  const clipPlacement = useFullPage
    ? {
        x: PAGE_CHART_PLACEMENT.offsetX,
        y: PAGE_CHART_PLACEMENT.offsetY,
        width: PAGE_CHART_PLACEMENT.width,
        height: PAGE_CHART_PLACEMENT.height
      }
